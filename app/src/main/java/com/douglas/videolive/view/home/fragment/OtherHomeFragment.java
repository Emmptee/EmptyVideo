...
import com.douglas.videolive.view.home.adapter.HomeNgBarAdapter;
import com.douglas.videolive.view.home.adapter.HomeNgBarViewPagerAdapter;
import com.douglas.videolive.view.home.adapter.HomeOtherAdapter;
 ...
 **/
public class OtherHomeFragment extends BaseFragment<HomeCateModelLogic, HomeCatePresenterImp> implements
        HomeCateContract.View, ViewPager.OnPageChangeListener {
    /**
     * 导航栏 分页
     */
////    小圆点指示器
...
//        return mOtherHomeFraments.get(arguments.getInt("position"));
    ...
    public void getOtherListRefresh(List<HomeRecommendHotCate> homeCates) {
        ...
        List<HomeRecommendHotCate> homeRecommendHotCates
                = new ArrayList<HomeRecommendHotCate>();
    ...
    private void getOtherColumnView(List<HomeRecommendHotCate> homeCates) {
        ...
        other_content_recyclerview.setLayoutManager(
...
//        导航栏
        haderView = adapter.setHeaderView(
                R.layout.view_viewpager, other_content_recyclerview);
        ngbarViewpager = (ViewPager) haderView.findViewById(R.id.ngbar_viewpager);
        Bundle arguments = getArguments();
        ngbarViewpager.removeOnPageChangeListener(
                mOtherHomeFraments.get(arguments.getInt("position")));
        ngbarViewpager.addOnPageChangeListener(
                mOtherHomeFraments.get(arguments.getInt("position")));
        ...
        for (int i = 0; i <mTotalPage; i++) {
            if (i <= 1) {
                GridView gridView = (GridView) inflater.inflate(
                        R.layout.view_layout_gridview, null);
                homeNgBarAdapter = new HomeNgBarAdapter(
                        getContext(), homeCates, i, mPageSize);
...
//            默认设置
                if (i == 0) {
                    imgView.setBackgroundResource(
                            R.mipmap.page__selected_indicator);
                } else {
                    imgView.setBackgroundResource(
                            R.mipmap.page__normal_indicator);
                }
                mIvpoints[i] = imgView;
//            设置边距
